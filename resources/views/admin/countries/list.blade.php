                    ...
                    { "name": "id",  
                        render: function ( data, type, row ) {
                            if (type === 'display' ) {
                                var btn='';
                                 @can('edit')
                                 btn+='<a  class="btn btn-outline-secondary" href="{{ request()->url() }}/'+row[4]+'/edit"><i class="icon-note"></i></a> &nbsp; &nbsp; ';
                                  @endcan
                                 @can('delete')
                                  btn+='<a class="btn btn-outline-danger" href="#" onclick="deleteData(\'{{ route('admin.'.request()->segment(2).'.destroy','') }}/'+row[4]+'\')"><i class="icon-trash"></i></a>';
                                  @endcan
